...
public class FirebaseIdentityProviderService implements IdentityProviderService {
    ...
    public void setCustomUserClaims(String uid, String role) {
        try {
            ...
            log.info("Successfully set custom claims for user {}: role={}", uid, role);
        } catch (FirebaseAuthException e) {
    ...
    public void deleteUser(String uid) {
        try {
            ...
            log.error("Error deleting Firebase user {}: {}", uid, e.getMessage());
            throw new RuntimeException("Failed to delete user from identity provider: " + e.getMessage(), e);
        }
    }
}
