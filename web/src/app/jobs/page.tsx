...
export default function JobsPage() {
  ...
  const deadlineLabel = (dateStr?: string) => {
    ...
    return <span className="text-gray-400 dark:text-gray-500 text-xs">Deadline: {d.toLocaleDateString()}</span>;
  };
        ...
        <div className="max-w-3xl mx-auto py-6 px-4">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Opportunities posted by alumni and the department</p>
            </div>
            {canPost && (
              <Link
                href="/jobs/create"
          ...
          {/* Filters */}
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 mb-6 flex flex-wrap gap-3 transition-colors">
            <input
              type="text"
              placeholder="Search by title or company…"
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="flex-1 min-w-48 text-sm border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100 focus:outline-none focus:border-blue-400 transition-colors"
            />
            <select
              value={type}
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 dark:text-gray-300 font-medium">No job postings found</p>
            ...
            <div className="space-y-3">
              {jobs.map(job => (
                <Link key={job.id} href={`/jobs/${job.id}`}>
                  <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 hover:border-blue-300 dark:hover:border-blue-500/50 hover:shadow-sm transition-all cursor-pointer">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <h2 className="font-semibold text-gray-900 dark:text-gray-100 truncate">{job.title}</h2>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-3">
                      ...
                      {job.salaryRange && (
                        <span className="text-xs text-gray-500 dark:text-gray-400">💰 {job.salaryRange}</span>
                      )}
                      <span className="text-xs text-gray-400 dark:text-gray-500 ml-auto">{timeAgo(job.createdAt)}</span>
                    </div>
              ...
              {hasMore && (
                <button
                  ...
                  className="w-full py-3 text-sm text-blue-600 dark:text-blue-300 font-medium border border-blue-200 dark:border-blue-800 rounded-xl hover:bg-blue-50 dark:hover:bg-blue-950/30 disabled:opacity-50 transition-colors"
                >
                  {loadingMore ? 'Loading…' : 'Load More'}
                </button>
              )}
              {!hasMore && jobs.length > 0 && (
                <p className="text-center text-sm text-gray-400 dark:text-gray-500 py-2">All postings loaded</p>
              )}
            </div>
          )}
        </div>
